#' @name .fit.diana
#' @title Divisive analysis for \code{tidyfit}
#' @description Calculates clusters using divisive analysis on a 'tidyFit' \code{R6} class. The function can be used with \code{\link{cluster}}.
#'
...
#' * metric
#'
#' The function provides a wrapper for \code{cluster::diana}. See \code{?diana} for more details.
#'
#' **Implementation**
#'
#' In addition to the arguments passed to \code{cluster::diana}, the function takes the argument \code{k} which specifies the number of clusters. An argument \code{transpose} is also provided (default \code{FALSE}). When \code{transpose = TRUE}, the data is transposed and clustering is done on \code{cor(data)} instead.
#'
...
#' fit <- m("diana",  ~ SMB + HML, data)
#' fit
#'
#' # Within 'cluster' function
#' fit <- cluster(data, ~ ., m("diana", k = 4), .mask = c("Date", "Industry"))
#' fitted(fit)
#'
#' # Cluster across columns
#' fit <- cluster(data, ~ ., m("diana", k = 4, transpose = TRUE), .mask = c("Date", "Industry"))
#' fitted(fit)
#'
