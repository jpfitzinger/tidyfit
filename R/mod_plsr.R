...
#' @description Fits a partial least squares regression and returns the results as a tibble. The function can be used with \code{regress}.
#'
...
#' @param ...  Additional arguments passed to \code{pls::plsr}.
#' @return A 'tibble'.
...
#' y <- rnorm(100)
#' fit <- mod_plsr(x, y, ncomp = 4)
#' fit
#'
#' @export
#'
#' @seealso \code{mod_pcr} method
#'
#' @importFrom pls plsr
...
mod_plsr <- function(
  ...
  if (is.null(x) & is.null(y)) {
    ...
    args <- append(args, list(.f = tidyfit::mod_plsr))
