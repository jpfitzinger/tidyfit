...
#' @description Fits a linear regression or classification with L2 penalty and returns the results as a tibble. The function can be used with \code{regress}.
#'
#' @details The ridge regression is estimated using \code{glmnet::glmnet} with \code{alpha = 0}. For classification pass \code{family = "binomial"} to \code{...}.
#'
...
#'
#' # Partial execution
#' mod <- mod_ridge(lambda = 1)
#' fit <- mod(x, y)
#' fit
#'
#' @export
#'
#' @seealso \code{mod_enet}, \code{mod_lasso} and \code{mod_´adalasso} methods
#'
...
mod_ridge <- function(x = NULL,
  ...
  if (is.null(x) & is.null(y)) {
    ...
    args <- append(args, list(.f = tidyfit::mod_ridge))
