...
In this short tutorial, I show how to calculate a rolling regression on grouped time series data using `tidyfit`. With very few lines of code, we will be able to estimate and analyse a large number of regressions. As usual, begin by loading necessary libraries:
...
While there are certainly very interesting insights to be had here, the aim of this walkthrough is not to interpret these results, but rather to demonstrate how a complex piece of analysis can be done very efficiently using the `tidyfit` package.
