...
Let's create a helper function to assess performance:
...
library(kableExtra)
tbl <- data.frame(
  `Best subset` = c("Forward selection (`leaps`)", "Forward selection (`leaps`)", "Backward selection (`leaps`)", ""),
...
The Bayesian regression achieves the highest out-of-sample performance, roughly on par with the adaptive Lasso and hierarchical feature regression. The subset selection algorithms achieve relatively good results, while the penalized and latent factor regressions perform worst. All methods improve on the OLS results.

## A glimpse at the backend

`tidyfit` makes it exceedingly simple to fit different regularized linear regressions. The package ensures that the input and output of the modeling engine, `tidyfit::m` are standardized and model results are comparable. For instance, whenever necessary, features are standardized with the coefficients transformed back to the original scale. Hyperparameter grids are set to reasonable starting values and can be passed manually to `tidyfit::m` as done in the case of the HFR above.

The package further includes a `tidyfit::classfiy` method that assumes a binomial response. Most of the methods implemented can handle Gaussian and binomial responses. Furthermore, the `tidyfit::regress` and `tidyfit::classify` methods seamlessly integrate with grouped tibbles, making it extremely powerful to estimate a large number of regressions for different data groupings. Groups as well as response distributions are automatically respected by the `tidyfit::cross_prod` prediction method.
