...
fit <- data_trn %>% 
  classify(Species ~ ., 
           ...
           `Neural Net` = m("nnet", size = 10, maxit = 200, decay = 0.1),
           .cv = "vfold_cv")
...
metrics %>% 
  ...
  mutate(model = as_factor(str_wrap(model, 11))) %>%
...
The least squares estimate performs poorest, while the random forest (nonlinear), neural network and the support vector machine (SVM) achieve the best results. The SVM is estimated with a linear kernel by default (use `kernel = <chosen_kernel>` to use a different kernel).
